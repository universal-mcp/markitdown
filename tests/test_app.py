...
def app_instance():
    ...
    return MarkitdownApp(integration=mock_integration)

def test_universal_mcp_markitdown_app_initialization(app_instance):
    """
    Test that the MarkitdownApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "markitdown", "MarkitdownApp instance has unexpected name."


def test_universal_mcp_markitdown_tool_docstrings_format(app_instance):
    """
    Test that each tool method in MarkitdownApp has a well-formatted docstring,
...
def test_universal_mcp_markitdown_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in MarkitdownApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
